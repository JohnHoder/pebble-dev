...
// Use uint16_t instead of uint32_t to save space. This wont be a problem until we have more than
// 2^16 timeline ids or resource ids.
...
""")
        for res in timeline_resources:
            ...
            output_file.write("  TIMELINE_RESOURCE_{} = {:#x},\n".format(res["name"], id))

        output_file.write("""} TimelineResourceId;

""")
        output_file.write("#define NUM_TIMELINE_RESOURCES {}\n".format(max_timeline_id + 1))

################################################################################################
def cmd_resource_header(args):
    dir = os.path.dirname(args.output_header)
