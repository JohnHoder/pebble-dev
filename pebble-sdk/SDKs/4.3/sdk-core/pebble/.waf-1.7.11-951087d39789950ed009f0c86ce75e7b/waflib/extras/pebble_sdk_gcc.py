...
def configure(conf):
	...
	optimize_flag='-Os'
	conf.load('gcc')
	pebble_cflags=['-std=c99','-mcpu=cortex-m3','-mthumb','-ffunction-sections','-fdata-sections','-g','-fPIE',optimize_flag]
	...
	pebble_linkflags=['-mcpu=cortex-m3','-mthumb','-Wl,--gc-sections','-Wl,--warn-common','-fPIE',optimize_flag]
...
def gen_inject_metadata_rule(bld,src_bin_file,dst_bin_file,elf_file,resource_file,timestamp,has_pkjs,has_worker):
	def inject_data_rule(task):
		...
		inject_metadata.inject_metadata(tgt_path,elf_path,res_path,timestamp,allow_js=has_pkjs,has_worker=has_worker)
