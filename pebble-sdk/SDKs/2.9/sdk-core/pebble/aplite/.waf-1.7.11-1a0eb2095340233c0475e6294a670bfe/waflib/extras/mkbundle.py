...
class PebbleBundle(object):
	...
	def add_watchapp(self,watchapp_path,app_timestamp,sdk_version):
		...
		self.bundle_manifest['application']={'timestamp':app_timestamp,'sdk_version':sdk_version,'name':os.path.basename(watchapp_path),'size':flen(watchapp_path),'crc':stm32crc(watchapp_path),}
	...
	def add_worker(self,worker_bin_path,worker_timestamp,sdk_version):
		...
		self.type='worker'
		self.bundle_manifest['worker']={'timestamp':worker_timestamp,'sdk_version':sdk_version,'name':worker_name,'size':worker_size,'crc':worker_crc,}
...
def make_watchapp_bundle(appinfo,sdk_version,watchapp=None,watchapp_timestamp=None,js_files=None,resources=None,resources_timestamp=None,worker_bin=None,outfile=None,verbose=False):
	bundle=PebbleBundle()
	...
	if worker_bin:
		worker_bin_path=os.path.expanduser(worker_bin)
		bundle.add_worker(worker_bin_path,watchapp_timestamp,sdk_version)
